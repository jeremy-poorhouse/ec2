...
function InstallPrep {
	...
	sudo apt-get update -qq && sudo apt-get upgrade -qq -y && sudo apt-get install -qq -y opencl-headers build-essential protobuf-compiler libprotoc-dev libboost-all-dev libleveldb-dev hdf5-tools libhdf5-serial-dev libopencv-core-dev  libopencv-highgui-dev libsnappy-dev libsnappy1 libatlas-base-dev cmake libstdc++6-4.8-dbg libgoogle-glog0 libgoogle-glog-dev libgflags-dev liblmdb-dev git python-pip gfortran

	sudo apt-get clean -qq && sudo apt-get install -qq -y linux-image-extra-`uname -r` linux-headers-`uname -r` linux-image-`uname -r`
}
...
function InstallCuda {
	...
	sudo apt-get update -qq && sudo apt-get install -qq -y cuda
...
function InstallBazel {
	...
	sudo apt-get update -qq && sudo apt-get install -qq -y oracle-java8-installer
	echo "deb http://storage.googleapis.com/bazel-apt stable jdk1.8" | sudo tee /etc/apt/sources.list.d/bazel.list
	curl https://storage.googleapis.com/bazel-apt/doc/apt-key.pub.gpg | sudo apt-key add -
	sudo apt-get update -qq && sudo apt-get install -qq -y bazel
}
